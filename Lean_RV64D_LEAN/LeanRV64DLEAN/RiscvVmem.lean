...
/-- Type quantifiers: k_ex373214# : Bool, level : Nat, k_ex373212# : Bool, k_ex373211# : Bool, sv_width
...
/-- Type quantifiers: tlb_index : Nat, k_ex373259# : Bool, k_ex373258# : Bool, sv_width : Nat, is_sv_mode(sv_width), 0
  ≤ tlb_index ∧ tlb_index ≤ (64 - 1) -/
def translate_TLB_hit (sv_width : Nat) (asid : (BitVec 9)) (vpn : (BitVec (sv_width - 12))) (ac : (AccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (ext_ptw : Unit) (tlb_index : Nat) (ent : TLB_Entry) : SailM (TR_Result (BitVec (bif sv_width
...
/-- Type quantifiers: k_ex373283# : Bool, k_ex373282# : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
def translate_TLB_miss (sv_width : Nat) (asid : (BitVec 9)) (base_ppn : (BitVec (bif sv_width = 32 then 22 else 44))) (vpn : (BitVec (sv_width - 12))) (ac : (AccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (ext_ptw : Unit) : SailM (TR_Result (BitVec (bif sv_width
  = 32 then 22 else 44)) PTW_Error) := do
  let initial_level :=
    bif (sv_width == 32)
    then 1
...
/-- Type quantifiers: k_ex373322# : Bool, k_ex373321# : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
def translate (sv_width : Nat) (asid : (BitVec 9)) (base_ppn : (BitVec (bif sv_width = 32 then 22 else 44))) (vpn : (BitVec (sv_width - 12))) (ac : (AccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (ext_ptw : Unit) : SailM (TR_Result (BitVec (bif sv_width
  = 32 then 22 else 44)) PTW_Error) := do
...
def translateAddr (vAddr : virtaddr) (ac : (AccessType Unit)) : SailM (TR_Result physaddr ExceptionType) := do
  ...
  bif (mode == Bare)
  then
    (pure (TR_Address ((Physaddr (zero_extend (m := 34) (bits_of_virtaddr vAddr))), init_ext_ptw)))
  else
    (do
      ...
      then (pure (TR_Failure ((translationException ac (PTW_Invalid_Addr ())), init_ext_ptw)))
        ...
        (do
          ...
          | .TR_Address (ppn, ext_ptw) =>
            ...
            (pure (TR_Address ((Physaddr (zero_extend (m := 34) paddr)), ext_ptw))))
